...
typedef struct {
    ...
    USHORT group;
    USHORT reserved[3];
} avs2_group_affinity_t;

typedef void (WINAPI *cond_func_t)(avs2_pthread_cond_t *cond);
typedef BOOL (WINAPI *cond_wait_t)(avs2_pthread_cond_t *cond, avs2_pthread_mutex_t *mutex, DWORD milliseconds);

typedef struct {
    /* global mutex for replacing MUTEX_INITIALIZER instances */
    avs2_pthread_mutex_t static_mutex;
...
} avs2_win32thread_control_t;

static avs2_win32thread_control_t thread_control;
...
static unsigned __stdcall avs2_win32thread_worker(void *arg)
{
    avs2_pthread_t *h = arg;
    h->ret = h->func(h->arg);
    return 0;
}
                        ...
                        void *(*start_routine)(void *), void *arg)
{
    ...
    thread->arg    = arg;
    thread->handle = (void *)_beginthreadex(NULL, 0, avs2_win32thread_worker, thread, 0, NULL);
...
{
    static avs2_pthread_mutex_t init = AVS2_PTHREAD_MUTEX_INITIALIZER;
    if (!memcmp(mutex, &init, sizeof(avs2_pthread_mutex_t))) {
...
typedef struct {
    avs2_pthread_mutex_t mtx_broadcast;
    avs2_pthread_mutex_t mtx_waiter_count;
    int waiter_count;
    HANDLE semaphore;
    HANDLE waiters_done;
    int is_broadcast;
...
int avs2_pthread_cond_init(avs2_pthread_cond_t *cond, const avs2_pthread_condattr_t *attr)
{
    ...
    win32_cond = calloc(1, sizeof(avs2_win32_cond_t));
    if (!win32_cond) {
        return -1;
    }
    cond->ptr = win32_cond;
    ...
    if (avs2_pthread_mutex_init(&win32_cond->mtx_waiter_count, NULL)) {
        return -1;
    }
    if (avs2_pthread_mutex_init(&win32_cond->mtx_broadcast, NULL)) {
        return -1;
    }
...
int avs2_pthread_cond_destroy(avs2_pthread_cond_t *cond)
{
    ...
    avs2_pthread_mutex_destroy(&win32_cond->mtx_broadcast);
    avs2_pthread_mutex_destroy(&win32_cond->mtx_waiter_count);
    free(win32_cond);
...
int avs2_pthread_cond_broadcast(avs2_pthread_cond_t *cond)
{
    ...
    /* non native condition variables */
    win32_cond = cond->ptr;
    avs2_pthread_mutex_lock(&win32_cond->mtx_broadcast);
    avs2_pthread_mutex_lock(&win32_cond->mtx_waiter_count);
    ...
    if (have_waiter) {
        ...
        avs2_pthread_mutex_unlock(&win32_cond->mtx_waiter_count);
        WaitForSingleObject(win32_cond->waiters_done, INFINITE);
        win32_cond->is_broadcast = 0;
    } else {
        avs2_pthread_mutex_unlock(&win32_cond->mtx_waiter_count);
    }
    return avs2_pthread_mutex_unlock(&win32_cond->mtx_broadcast);
}
...
int avs2_pthread_cond_wait(avs2_pthread_cond_t *cond, avs2_pthread_mutex_t *mutex)
{
    ...
    avs2_pthread_mutex_lock(&win32_cond->mtx_broadcast);
    avs2_pthread_mutex_unlock(&win32_cond->mtx_broadcast);
    ...
    avs2_pthread_mutex_unlock(mutex);
    ...
    return avs2_pthread_mutex_lock(mutex);
}
...
int avs2_win32_threading_init(void)
{
    ...
    return avs2_pthread_mutex_init(&thread_control.static_mutex, NULL);
}

void avs2_win32_threading_destroy(void)
{
    avs2_pthread_mutex_destroy(&thread_control.static_mutex);
    memset(&thread_control, 0, sizeof(avs2_win32thread_control_t));
}

int avs2_pthread_num_processors_np()
{
    ...
    BOOL (*get_thread_affinity)(HANDLE thread, avs2_group_affinity_t * group_affinity) = (void *)GetProcAddress(kernel_dll, "GetThreadGroupAffinity");
    if (get_thread_affinity) {
        /* running on a platform that supports >64 logical cpus */
        avs2_group_affinity_t thread_affinity;
