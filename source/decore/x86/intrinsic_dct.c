...
void idct_4x4_sse128(coef_t *blk, int shift, int clip)
{
    ...
    m128iS0 = _mm_loadu_si128((__m128i*)&blk[0]);
    m128iS1 = _mm_loadu_si128((__m128i*)&blk[8]);
    ...
    c32_rnd = _mm_set1_epi32(shift ? (1 << (shift - 1)) : 0);						//设置四舍五入
    ...
    m128iS0 = _mm_packs_epi32(_mm_srai_epi32(_mm_add_epi32(E0, O0), shift), _mm_srai_epi32(_mm_sub_epi32(E1, O1), shift));
    m128iS1 = _mm_packs_epi32(_mm_srai_epi32(_mm_add_epi32(E1, O1), shift), _mm_srai_epi32(_mm_sub_epi32(E0, O0), shift));
    ...
    _mm_storeu_si128((__m128i*)&blk[0], m128iS0);
    _mm_storeu_si128((__m128i*)&blk[8], m128iS1);
}
...
void idct_4x16_sse128(coef_t *blk, int shift, int clip)
{
    ...
    in00 = _mm_loadu_si128((const __m128i*)&blk[0 * 4]);	// [07 06 05 04 03 02 01 00]
    in01 = _mm_loadu_si128((const __m128i*)&blk[2 * 4]); // [27 26 25 24 23 22 21 20]
    in02 = _mm_loadu_si128((const __m128i*)&blk[4 * 4]);   // [47 46 45 44 43 42 41 40]
    in03 = _mm_loadu_si128((const __m128i*)&blk[6 * 4]);    // [67 66 65 64 63 62 61 60]
    in04 = _mm_loadu_si128((const __m128i*)&blk[8 * 4]);
    in05 = _mm_loadu_si128((const __m128i*)&blk[10 * 4]);
    in06 = _mm_loadu_si128((const __m128i*)&blk[12 * 4]);
    in07 = _mm_loadu_si128((const __m128i*)&blk[14 * 4]);
...
#undef COMPUTE_ROW
    ...
    //transpose matrix .
    {
        ...
        E01 = _mm_madd_epi16(m128Tmp0, c16_p32_p32);
        E01 = _mm_add_epi32(E01, c32_rnd);
        E11 = _mm_madd_epi16(m128Tmp0, c16_n32_p32);
        E11 = _mm_add_epi32(E11, c32_rnd);

        m128Tmp1 = _mm_unpackhi_epi16(res00, res04);
        E02 = _mm_madd_epi16(m128Tmp1, c16_p32_p32);
        E02 = _mm_add_epi32(E02, c32_rnd);
        E12 = _mm_madd_epi16(m128Tmp1, c16_n32_p32);
        E12 = _mm_add_epi32(E12, c32_rnd);

        m128Tmp0 = _mm_unpacklo_epi16(res01, res05);
        E03 = _mm_madd_epi16(m128Tmp0, c16_p32_p32);
        E03 = _mm_add_epi32(E03, c32_rnd);
        E13 = _mm_madd_epi16(m128Tmp0, c16_n32_p32);
        E13 = _mm_add_epi32(E13, c32_rnd);

        m128Tmp1 = _mm_unpackhi_epi16(res01, res05);
        E04 = _mm_madd_epi16(m128Tmp1, c16_p32_p32);
        E04 = _mm_add_epi32(E04, c32_rnd);
        E14 = _mm_madd_epi16(m128Tmp1, c16_n32_p32);
        E14 = _mm_add_epi32(E14, c32_rnd);
        ...
        res00 = _mm_packs_epi32(_mm_srai_epi32(_mm_add_epi32(E01, O01), shift), _mm_srai_epi32(_mm_add_epi32(E02, O02), shift));
        res01 = _mm_packs_epi32(_mm_srai_epi32(_mm_add_epi32(E03, O03), shift), _mm_srai_epi32(_mm_add_epi32(E04, O04), shift));

        res06 = _mm_packs_epi32(_mm_srai_epi32(_mm_sub_epi32(E01, O01), shift), _mm_srai_epi32(_mm_sub_epi32(E02, O02), shift));
        res07 = _mm_packs_epi32(_mm_srai_epi32(_mm_sub_epi32(E03, O03), shift), _mm_srai_epi32(_mm_sub_epi32(E04, O04), shift));


        res02 = _mm_packs_epi32(_mm_srai_epi32(_mm_add_epi32(E11, O11), shift), _mm_srai_epi32(_mm_add_epi32(E12, O12), shift));
        res03 = _mm_packs_epi32(_mm_srai_epi32(_mm_add_epi32(E13, O13), shift), _mm_srai_epi32(_mm_add_epi32(E14, O14), shift));

        res04 = _mm_packs_epi32(_mm_srai_epi32(_mm_sub_epi32(E11, O11), shift), _mm_srai_epi32(_mm_sub_epi32(E12, O12), shift));
        res05 = _mm_packs_epi32(_mm_srai_epi32(_mm_sub_epi32(E13, O13), shift), _mm_srai_epi32(_mm_sub_epi32(E14, O14), shift));
    ...
    //clip
    {
        const __m128i max_val = _mm_set1_epi16((1 << (clip - 1)) - 1);
        const __m128i min_val = _mm_set1_epi16(-(1 << (clip - 1)));
        res00 = _mm_min_epi16(res00, max_val);
        res00 = _mm_max_epi16(res00, min_val);
        res02 = _mm_min_epi16(res02, max_val);
        res02 = _mm_max_epi16(res02, min_val);
        res04 = _mm_min_epi16(res04, max_val);
        res04 = _mm_max_epi16(res04, min_val);
        res06 = _mm_min_epi16(res06, max_val);
        res06 = _mm_max_epi16(res06, min_val);
        res01 = _mm_min_epi16(res01, max_val);
        res01 = _mm_max_epi16(res01, min_val);
        res03 = _mm_min_epi16(res03, max_val);
        res03 = _mm_max_epi16(res03, min_val);
        res05 = _mm_min_epi16(res05, max_val);
        res05 = _mm_max_epi16(res05, min_val);
        res07 = _mm_min_epi16(res07, max_val);
        res07 = _mm_max_epi16(res07, min_val);
    }

    _mm_storeu_si128((__m128i*)&blk[0], res00);
    _mm_storeu_si128((__m128i*)&blk[8], res02);
    _mm_storeu_si128((__m128i*)&blk[16], res04);
    _mm_storeu_si128((__m128i*)&blk[24], res06);
    _mm_storeu_si128((__m128i*)&blk[32], res01);
    _mm_storeu_si128((__m128i*)&blk[40], res03);
    _mm_storeu_si128((__m128i*)&blk[48], res05);
    _mm_storeu_si128((__m128i*)&blk[56], res07);
...
void idct_16x4_sse128(coef_t *blk, int shift, int clip)
{
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        in00[i] = _mm_loadu_si128((const __m128i*)&blk[0 * 16 + offset]);	// [07 06 05 04 03 02 01 00]
        in01[i] = _mm_loadu_si128((const __m128i*)&blk[1 * 16 + offset]);  // [17 16 15 14 13 12 11 10]
        in02[i] = _mm_loadu_si128((const __m128i*)&blk[2 * 16 + offset]); // [27 26 25 24 23 22 21 20]
        in03[i] = _mm_loadu_si128((const __m128i*)&blk[3 * 16 + offset]);     // [37 36 35 34 33 32 31 30]
    }
    ...
    for (part = 0; part < 2; part++)
    {
        ...
        E0A = _mm_madd_epi16(T_00_01A, c16_p32_p32);
        E1A = _mm_madd_epi16(T_00_01A, c16_n32_p32);
        E0A = _mm_add_epi32(E0A, c32_rnd);
        E1A = _mm_add_epi32(E1A, c32_rnd);

        E0B = _mm_madd_epi16(T_00_01B, c16_p32_p32);
        E1B = _mm_madd_epi16(T_00_01B, c16_n32_p32);
        E0B = _mm_add_epi32(E0B, c32_rnd);
        E1B = _mm_add_epi32(E1B, c32_rnd);
...
#undef COMPUTE_ROW
    ...
    _mm_storeu_si128((__m128i*)&blk[0], res00[0]);
    _mm_storeu_si128((__m128i*)&blk[8], res00[1]);
    _mm_storeu_si128((__m128i*)&blk[16], res01[0]);
    _mm_storeu_si128((__m128i*)&blk[24], res01[1]);
    _mm_storeu_si128((__m128i*)&blk[32], res02[0]);
    _mm_storeu_si128((__m128i*)&blk[40], res02[1]);
    _mm_storeu_si128((__m128i*)&blk[48], res03[0]);
    _mm_storeu_si128((__m128i*)&blk[56], res03[1]);
}
...
void idct_8x8_sse128(coef_t *blk, int shift, int clip)
{
    ...
    m128iAdd = _mm_set1_epi32(shift ? (1 << (shift - 1)) : 0);						//设置四舍五入
...
void idct_16x16_sse128(coef_t *blk, int shift, int clip)
{
    ...
    for (pass = 0; pass < 2; pass++)
    {
        if (pass == 1)
        {
            c32_rnd = _mm_set1_epi32(shift ? (1 << (shift - 1)) : 0);				// pass == 1 第二次四舍五入
            nShift = shift;
        }
...
{
    __m128i m128iS0[4], m128iS1[4], m128iS2[4], m128iS3[4], m128iS4[4], m128iS5[4], m128iS6[4], m128iS7[4], m128iAdd, m128Tmp0, m128Tmp1, m128Tmp2, m128Tmp3, E0h, E1h, E2h, E3h, E0l, E1l, E2l, E3l, O0h, O1h, O2h, O3h, O0l, O1l, O2l, O3l, EE0l, EE1l, E00l, E01l, EE0h, EE1h, E00h, E01h;
    int i, pass;
    ...
    {
        m128iS1[pass] = _mm_load_si128((__m128i*)&blk[pass * 8 + 1 * 32]);
        m128iS3[pass] = _mm_load_si128((__m128i*)&blk[pass * 8 + 3 * 32]);
        ...
        m128iS5[pass] = _mm_load_si128((__m128i*)&blk[pass * 8 + 5 * 32]);
        m128iS7[pass] = _mm_load_si128((__m128i*)&blk[pass * 8 + 7 * 32]);
        ...
        m128iS0[pass] = _mm_load_si128((__m128i*)&blk[pass * 8 + 0 * 32]);
        m128iS4[pass] = _mm_load_si128((__m128i*)&blk[pass * 8 + 4 * 32]);
        ...
        m128iS2[pass] = _mm_load_si128((__m128i*)&blk[pass * 8 + 2 * 32]);
        m128iS6[pass] = _mm_load_si128((__m128i*)&blk[pass * 8 + 6 * 32]);
...

    {
        ...
        __m128i c32_rnd = _mm_set1_epi32(shift ? (1 << (shift - 1)) : 0);				// pass == 1 第二次四舍五入
        int nShift = shift;
...
#define STORE_LINE(L0, L1, L2, L3, offsetV) \
    _mm_store_si128((__m128i*)&blk[0 + offsetV * 32], L0); \
    _mm_store_si128((__m128i*)&blk[8 + offsetV * 32], L1); \
    _mm_store_si128((__m128i*)&blk[16 + offsetV * 32], L2); \
    _mm_store_si128((__m128i*)&blk[24 + offsetV * 32], L3);
...
void idct_8x32_sse128(coef_t *blk, int shift, int clip)
{
    ...
    in00 = _mm_load_si128((const __m128i*)&blk[0 * 8]);
    in01 = _mm_load_si128((const __m128i*)&blk[1 * 8]);
    in02 = _mm_load_si128((const __m128i*)&blk[2 * 8]);
    in03 = _mm_load_si128((const __m128i*)&blk[3 * 8]);
    in04 = _mm_load_si128((const __m128i*)&blk[4 * 8]);
    in05 = _mm_load_si128((const __m128i*)&blk[5 * 8]);
    in06 = _mm_load_si128((const __m128i*)&blk[6 * 8]);
    in07 = _mm_load_si128((const __m128i*)&blk[7 * 8]);
    in08 = _mm_load_si128((const __m128i*)&blk[8 * 8]);
    in09 = _mm_load_si128((const __m128i*)&blk[9 * 8]);
    in10 = _mm_load_si128((const __m128i*)&blk[10 * 8]);
    in11 = _mm_load_si128((const __m128i*)&blk[11 * 8]);
    in12 = _mm_load_si128((const __m128i*)&blk[12 * 8]);
    in13 = _mm_load_si128((const __m128i*)&blk[13 * 8]);
    in14 = _mm_load_si128((const __m128i*)&blk[14 * 8]);
    in15 = _mm_load_si128((const __m128i*)&blk[15 * 8]);
    in16 = _mm_load_si128((const __m128i*)&blk[16 * 8]);
    in17 = _mm_load_si128((const __m128i*)&blk[17 * 8]);
    in18 = _mm_load_si128((const __m128i*)&blk[18 * 8]);
    in19 = _mm_load_si128((const __m128i*)&blk[19 * 8]);
    in20 = _mm_load_si128((const __m128i*)&blk[20 * 8]);
    in21 = _mm_load_si128((const __m128i*)&blk[21 * 8]);
    in22 = _mm_load_si128((const __m128i*)&blk[22 * 8]);
    in23 = _mm_load_si128((const __m128i*)&blk[23 * 8]);
    in24 = _mm_load_si128((const __m128i*)&blk[24 * 8]);
    in25 = _mm_load_si128((const __m128i*)&blk[25 * 8]);
    in26 = _mm_load_si128((const __m128i*)&blk[26 * 8]);
    in27 = _mm_load_si128((const __m128i*)&blk[27 * 8]);
    in28 = _mm_load_si128((const __m128i*)&blk[28 * 8]);
    in29 = _mm_load_si128((const __m128i*)&blk[29 * 8]);
    in30 = _mm_load_si128((const __m128i*)&blk[30 * 8]);
    in31 = _mm_load_si128((const __m128i*)&blk[31 * 8]);
...
{
    __m128i	tmpZero, tmpCoef0, tmpCoef1, tmpCoef2, tmpCoef3, tmpLoad0, tmpLoad1;
    ...
    rnd_factor = shift == 0 ? 0 : 1 << (shift - 1);
    factor = _mm_set1_epi32(rnd_factor);
    tmpZero = _mm_setzero_si128();							//0 elements

    //load coef data, a matrix of 4x4
    tmpLoad0 = _mm_loadu_si128((__m128i*)&coef[0][0]);		//coef[0][] & coef[1][]
    tmpLoad1 = _mm_loadu_si128((__m128i*)&coef[2][0]);		//coef[2][] & coef[3][]
    tmpCoef0 = _mm_unpacklo_epi16(tmpLoad0, tmpZero);		//coef[0][]
    tmpCoef1 = _mm_unpackhi_epi16(tmpLoad0, tmpZero);		//coef[1][]
    tmpCoef2 = _mm_unpacklo_epi16(tmpLoad1, tmpZero);		//coef[2][]
    tmpCoef3 = _mm_unpackhi_epi16(tmpLoad1, tmpZero);		//coef[3][]

    /*-------for j = 0-------*/
    //load blk data
    tmpSrc = blk[0];
    tmpS0 = _mm_set1_epi32(tmpSrc);							//tmpSrc[0][0]			
    tmpSrc = blk[1];
    tmpS1 = _mm_set1_epi32(tmpSrc);							//tmpSrc[0][1]
    tmpSrc = blk[2];
    tmpS2 = _mm_set1_epi32(tmpSrc);
    tmpSrc = blk[3];
    tmpS3 = _mm_set1_epi32(tmpSrc);
    //mutiple & add
    ...
    //shift operation
    tmpDst0 = _mm_srai_epi32(tmpSum3, shift);
    //clip3 operation
    tmpDst1 = _mm_packs_epi32(tmpDst0, tmpZero);			//!only low 64bits (4xSHORT) are valid!

    /*-------for j = 1-------*/
    //load blk data
    tmpSrc = blk[0 + i_blk];
    tmpS0 = _mm_set1_epi32(tmpSrc);							//tmpSrc[1][0]
    tmpSrc = blk[1 + i_blk];
    tmpS1 = _mm_set1_epi32(tmpSrc);							//tmpSrc[1][1]
    tmpSrc = blk[2 + i_blk];
    tmpS2 = _mm_set1_epi32(tmpSrc);
    tmpSrc = blk[3 + i_blk];
    tmpS3 = _mm_set1_epi32(tmpSrc);
    ...
    tmpDst0 = _mm_srai_epi32(tmpSum3, shift);
    //clip3 operation
    tmpDst2 = _mm_packs_epi32(tmpDst0, tmpZero);			//!only low 64bits (4xSHORT) are valid!

    /*-------for j = 2-------*/
    //load blk data
    tmpSrc = blk[0 + 2 * i_blk];
    tmpS0 = _mm_set1_epi32(tmpSrc);							//tmpSrc[2][0]
    tmpSrc = blk[1 + 2 * i_blk];
    tmpS1 = _mm_set1_epi32(tmpSrc);							//tmpSrc[2][1]
    tmpSrc = blk[2 + 2 * i_blk];
    tmpS2 = _mm_set1_epi32(tmpSrc);
    tmpSrc = blk[3 + 2 * i_blk];
    tmpS3 = _mm_set1_epi32(tmpSrc);
    //mutiple & add
    tmpProduct0 = _mm_madd_epi16(tmpCoef0, tmpS0);
    tmpProduct1 = _mm_madd_epi16(tmpCoef1, tmpS1);
    ...
    tmpDst0 = _mm_srai_epi32(tmpSum3, shift);
    //clip3 operation
    tmpDst3 = _mm_packs_epi32(tmpDst0, tmpZero);			//!only low 64bits (4xSHORT) are valid!

    /*-------for j = 3-------*/
    //load blk data
    tmpSrc = blk[0 + 3 * i_blk];
    tmpS0 = _mm_set1_epi32(tmpSrc);							//tmpSrc[2][0]
    tmpSrc = blk[1 + 3 * i_blk];
    tmpS1 = _mm_set1_epi32(tmpSrc);							//tmpSrc[2][1]
    tmpSrc = blk[2 + 3 * i_blk];
    tmpS2 = _mm_set1_epi32(tmpSrc);
    tmpSrc = blk[3 + 3 * i_blk];
    tmpS3 = _mm_set1_epi32(tmpSrc);
    //mutiple & add
    tmpProduct0 = _mm_madd_epi16(tmpCoef0, tmpS0);
    tmpProduct1 = _mm_madd_epi16(tmpCoef1, tmpS1);
    ...
    tmpDst0 = _mm_srai_epi32(tmpSum3, shift);
    //clip3 operation
    tmpDst4 = _mm_packs_epi32(tmpDst0, tmpZero);				//!only low 64bits (4xSHORT) are valid!

    //store blk data
    _mm_storel_epi64((__m128i*)&blk[0 * i_blk + 0], tmpDst1);	//store from &blk[0]
    _mm_storel_epi64((__m128i*)&blk[1 * i_blk + 0], tmpDst2);
    _mm_storel_epi64((__m128i*)&blk[2 * i_blk + 0], tmpDst3);
    _mm_storel_epi64((__m128i*)&blk[3 * i_blk + 0], tmpDst4);
}


void inv_2nd_trans_ver_sse128(coef_t *blk, int i_blk, int shift, const i16s_t coef[4][4])
{
    __m128i	tmpZero, tmpSrc0, tmpSrc1, tmpSrc2, tmpSrc3, tmpLoad0, tmpLoad1, tmpLoad2, tmpLoad3;
    ...
    //load blk data
    tmpLoad0 = _mm_loadu_si128((__m128i*)&blk[0]);
    tmpLoad1 = _mm_loadu_si128((__m128i*)&blk[1 * i_blk]);
    tmpLoad2 = _mm_loadu_si128((__m128i*)&blk[2 * i_blk]);
    tmpLoad3 = _mm_loadu_si128((__m128i*)&blk[3 * i_blk]);
    tmpSrc0 = _mm_unpacklo_epi16(tmpLoad0, tmpZero);			//tmpSrc[0][]
    tmpSrc1 = _mm_unpacklo_epi16(tmpLoad1, tmpZero);			//tmpSrc[1][]
    tmpSrc2 = _mm_unpacklo_epi16(tmpLoad2, tmpZero);			//tmpSrc[2][]
    tmpSrc3 = _mm_unpacklo_epi16(tmpLoad3, tmpZero);			//tmpSrc[3][]

    /*-------for i = 0-------*/
    //load coef data
    tmpCoef = coef[0][0];
    tmpC0 = _mm_set1_epi32(tmpCoef);							//tmpCoef[0][0]
    tmpCoef = coef[1][0];
    tmpC1 = _mm_set1_epi32(tmpCoef);							//tmpCoef[1][0]
    tmpCoef = coef[2][0];
    tmpC2 = _mm_set1_epi32(tmpCoef);
    tmpCoef = coef[3][0];
    tmpC3 = _mm_set1_epi32(tmpCoef);
    //mutiple & add
    ...
    //add operation
    tmpSum0 = _mm_add_epi32(tmpProduct0, tmpProduct1);			//tmpSum0.m128i_i32[0];
    tmpSum1 = _mm_add_epi32(tmpProduct2, tmpProduct3);
    tmpSum2 = _mm_add_epi32(tmpSum0, tmpSum1);
    tmpSum3 = _mm_add_epi32(tmpSum2, factor);
    //shift operation
    ...
    //load coef data
    tmpCoef = coef[0][1];
    tmpC0 = _mm_set1_epi32(tmpCoef);							//tmpCoef[0][1]
    tmpCoef = coef[1][1];
    tmpC1 = _mm_set1_epi32(tmpCoef);							//tmpCoef[1][1]
    tmpCoef = coef[2][1];
    tmpC2 = _mm_set1_epi32(tmpCoef);
    tmpCoef = coef[3][1];
    tmpC3 = _mm_set1_epi32(tmpCoef);
...
void inv_2nd_trans_sse128(coef_t *blk, int i_blk, int shift, int clip_depth, const i16s_t coef[4][4])
{
	...
	__m128i tmpS0, tmpS1, tmpS2, tmpS3;
	__m128i tmpProduct0, tmpProduct1, tmpProduct2, tmpProduct3, tmpSum0, tmpSum1, tmpSum2, tmpSum3, tmpDst0, tmpDst1, tmpDst2, tmpDst3, tmpDst4;
	__m128i factor, min, max;
	int rnd_factor, min_val, max_val;
	coef_t tmpCoef, tmpSrc;
	...
	tmpSum3 = _mm_add_epi32(tmpSum2, factor);
	//shift operation
	tmpDst0 = _mm_srai_epi32(tmpSum3, 5);
	//clip3 operation
	tmpDst1 = _mm_packs_epi32(tmpDst0, tmpZero);				//!only low 64bits (4xSHORT) are valid!
	...
	max_val = (1 << (clip_depth - 1)) - 1;
	min_val = -max_val - 1;
	max = _mm_set1_epi32(max_val);
	min = _mm_set1_epi32(min_val);
	...
	tmpDst0 = _mm_srai_epi32(tmpSum3, shift);
	//clip3 operation
    tmpDst0 = _mm_min_epi32(tmpDst0, max);
    tmpDst0 = _mm_max_epi32(tmpDst0, min);
...
void inv_wavelet_B64_sse128(coef_t *blk)
{
    ...
    for (i = 0; i < 4; i++){
        T00[i] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 * i + 32 * 0]), 1);
        T01[i] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 * i + 32 * 1]), 1);
        T02[i] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 * i + 32 * 2]), 1);
        T03[i] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 * i + 32 * 3]), 1);
        T04[i] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 * i + 32 * 4]), 1);
        T05[i] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 * i + 32 * 5]), 1);
        T06[i] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 * i + 32 * 6]), 1);
        T07[i] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 * i + 32 * 7]), 1);

        T08[i] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 * i + 32 * 8]), 1);
        T09[i] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 * i + 32 * 9]), 1);
        T10[i] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 * i + 32 * 10]), 1);
        T11[i] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 * i + 32 * 11]), 1);
        T12[i] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 * i + 32 * 12]), 1);
        T13[i] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 * i + 32 * 13]), 1);
        T14[i] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 * i + 32 * 14]), 1);
        T15[i] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 * i + 32 * 15]), 1);

        T16[i] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 * i + 32 * 16]), 1);
        T17[i] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 * i + 32 * 17]), 1);
        T18[i] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 * i + 32 * 18]), 1);
        T19[i] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 * i + 32 * 19]), 1);
        T20[i] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 * i + 32 * 20]), 1);
        T21[i] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 * i + 32 * 21]), 1);
        T22[i] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 * i + 32 * 22]), 1);
        T23[i] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 * i + 32 * 23]), 1);

        T24[i] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 * i + 32 * 24]), 1);
        T25[i] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 * i + 32 * 25]), 1);
        T26[i] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 * i + 32 * 26]), 1);
        T27[i] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 * i + 32 * 27]), 1);
        T28[i] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 * i + 32 * 28]), 1);
        T29[i] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 * i + 32 * 29]), 1);
        T30[i] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 * i + 32 * 30]), 1);
        T31[i] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 * i + 32 * 31]), 1);
    }
    ...
    //store
    for (i = 0; i < 8; i++){
        _mm_store_si128((__m128i*)&blk[8 * i], T00[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64], T01[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 2], T02[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 3], T03[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 4], T04[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 5], T05[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 6], T06[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 7], T07[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 8], T08[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 9], T09[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 10], T10[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 11], T11[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 12], T12[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 13], T13[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 14], T14[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 15], T15[i]);

        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 16], T16[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 17], T17[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 18], T18[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 19], T19[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 20], T20[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 21], T21[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 22], T22[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 23], T23[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 24], T24[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 25], T25[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 26], T26[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 27], T27[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 28], T28[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 29], T29[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 30], T30[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 31], T31[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 32], T32[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 33], T33[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 34], T34[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 35], T35[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 36], T36[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 37], T37[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 38], T38[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 39], T39[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 40], T40[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 41], T41[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 42], T42[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 43], T43[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 44], T44[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 45], T45[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 46], T46[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 47], T47[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 48], T48[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 49], T49[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 50], T50[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 51], T51[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 52], T52[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 53], T53[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 54], T54[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 55], T55[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 56], T56[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 57], T57[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 58], T58[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 59], T59[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 60], T60[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 61], T61[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 62], T62[i]);
        _mm_store_si128((__m128i*)&blk[8 * i + 64 * 63], T63[i]);
    }
}
...
void inv_wavelet_NSQT_Hor_sse128(coef_t *blk)
{
    ...
    /*--vertical transform--*/
    //32*8, LOAD AND SHIFT
    T00[0] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[0 + 32 * 0]), 1);
    T01[0] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[0 + 32 * 1]), 1);
    T02[0] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[0 + 32 * 2]), 1);
    T03[0] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[0 + 32 * 3]), 1);
    T04[0] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[0 + 32 * 4]), 1);
    T05[0] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[0 + 32 * 5]), 1);
    T06[0] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[0 + 32 * 6]), 1);
    T07[0] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[0 + 32 * 7]), 1);

    T00[1] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 + 32 * 0]), 1);
    T01[1] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 + 32 * 1]), 1);
    T02[1] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 + 32 * 2]), 1);
    T03[1] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 + 32 * 3]), 1);
    T04[1] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 + 32 * 4]), 1);
    T05[1] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 + 32 * 5]), 1);
    T06[1] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 + 32 * 6]), 1);
    T07[1] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[8 + 32 * 7]), 1);

    T00[2] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[16 + 32 * 0]), 1);
    T01[2] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[16 + 32 * 1]), 1);
    T02[2] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[16 + 32 * 2]), 1);
    T03[2] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[16 + 32 * 3]), 1);
    T04[2] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[16 + 32 * 4]), 1);
    T05[2] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[16 + 32 * 5]), 1);
    T06[2] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[16 + 32 * 6]), 1);
    T07[2] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[16 + 32 * 7]), 1);

    T00[3] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[24 + 32 * 0]), 1);
    T01[3] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[24 + 32 * 1]), 1);
    T02[3] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[24 + 32 * 2]), 1);
    T03[3] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[24 + 32 * 3]), 1);
    T04[3] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[24 + 32 * 4]), 1);
    T05[3] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[24 + 32 * 5]), 1);
    T06[3] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[24 + 32 * 6]), 1);
    T07[3] = _mm_srai_epi16(_mm_load_si128((__m128i*)&blk[24 + 32 * 7]), 1);
...
void inv_wavelet_NSQT_Ver_sse128(coef_t *blk)
{
	...
	/*--load & shift--*/
	//8*32
	S00 = _mm_srai_epi16(_mm_loadu_si128((__m128i*)&blk[8 * 0]), 1);
	S01 = _mm_srai_epi16(_mm_loadu_si128((__m128i*)&blk[8 * 1]), 1);
	S02 = _mm_srai_epi16(_mm_loadu_si128((__m128i*)&blk[8 * 2]), 1);
	S03 = _mm_srai_epi16(_mm_loadu_si128((__m128i*)&blk[8 * 3]), 1);
	S04 = _mm_srai_epi16(_mm_loadu_si128((__m128i*)&blk[8 * 4]), 1);
	S05 = _mm_srai_epi16(_mm_loadu_si128((__m128i*)&blk[8 * 5]), 1);
	S06 = _mm_srai_epi16(_mm_loadu_si128((__m128i*)&blk[8 * 6]), 1);
	S07 = _mm_srai_epi16(_mm_loadu_si128((__m128i*)&blk[8 * 7]), 1);
	S08 = _mm_srai_epi16(_mm_loadu_si128((__m128i*)&blk[8 * 8]), 1);
	S09 = _mm_srai_epi16(_mm_loadu_si128((__m128i*)&blk[8 * 9]), 1);
	S10 = _mm_srai_epi16(_mm_loadu_si128((__m128i*)&blk[8 * 10]), 1);
	S11 = _mm_srai_epi16(_mm_loadu_si128((__m128i*)&blk[8 * 11]), 1);
	S12 = _mm_srai_epi16(_mm_loadu_si128((__m128i*)&blk[8 * 12]), 1);
	S13 = _mm_srai_epi16(_mm_loadu_si128((__m128i*)&blk[8 * 13]), 1);
	S14 = _mm_srai_epi16(_mm_loadu_si128((__m128i*)&blk[8 * 14]), 1);
	S15 = _mm_srai_epi16(_mm_loadu_si128((__m128i*)&blk[8 * 15]), 1);
	S16 = _mm_srai_epi16(_mm_loadu_si128((__m128i*)&blk[8 * 16]), 1);
	S17 = _mm_srai_epi16(_mm_loadu_si128((__m128i*)&blk[8 * 17]), 1);
	S18 = _mm_srai_epi16(_mm_loadu_si128((__m128i*)&blk[8 * 18]), 1);
	S19 = _mm_srai_epi16(_mm_loadu_si128((__m128i*)&blk[8 * 19]), 1);
	S20 = _mm_srai_epi16(_mm_loadu_si128((__m128i*)&blk[8 * 20]), 1);
	S21 = _mm_srai_epi16(_mm_loadu_si128((__m128i*)&blk[8 * 21]), 1);
	S22 = _mm_srai_epi16(_mm_loadu_si128((__m128i*)&blk[8 * 22]), 1);
	S23 = _mm_srai_epi16(_mm_loadu_si128((__m128i*)&blk[8 * 23]), 1);
	S24 = _mm_srai_epi16(_mm_loadu_si128((__m128i*)&blk[8 * 24]), 1);
	S25 = _mm_srai_epi16(_mm_loadu_si128((__m128i*)&blk[8 * 25]), 1);
	S26 = _mm_srai_epi16(_mm_loadu_si128((__m128i*)&blk[8 * 26]), 1);
	S27 = _mm_srai_epi16(_mm_loadu_si128((__m128i*)&blk[8 * 27]), 1);
	S28 = _mm_srai_epi16(_mm_loadu_si128((__m128i*)&blk[8 * 28]), 1);
	S29 = _mm_srai_epi16(_mm_loadu_si128((__m128i*)&blk[8 * 29]), 1);
	S30 = _mm_srai_epi16(_mm_loadu_si128((__m128i*)&blk[8 * 30]), 1);
	S31 = _mm_srai_epi16(_mm_loadu_si128((__m128i*)&blk[8 * 31]), 1);
	...
	//16x64
	for (i = 0; i < 2; i++){
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 0], V00[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 1], V01[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 2], V02[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 3], V03[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 4], V04[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 5], V05[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 6], V06[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 7], V07[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 8], V08[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 9], V09[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 10], V10[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 11], V11[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 12], V12[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 13], V13[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 14], V14[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 15], V15[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 16], V16[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 17], V17[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 18], V18[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 19], V19[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 20], V20[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 21], V21[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 22], V22[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 23], V23[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 24], V24[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 25], V25[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 26], V26[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 27], V27[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 28], V28[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 29], V29[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 30], V30[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 31], V31[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 32], V32[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 33], V33[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 34], V34[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 35], V35[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 36], V36[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 37], V37[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 38], V38[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 39], V39[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 40], V40[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 41], V41[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 42], V42[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 43], V43[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 44], V44[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 45], V45[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 46], V46[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 47], V47[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 48], V48[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 49], V49[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 50], V50[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 51], V51[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 52], V52[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 53], V53[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 54], V54[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 55], V55[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 56], V56[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 57], V57[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 58], V58[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 59], V59[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 60], V60[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 61], V61[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 62], V62[i]);
		_mm_storeu_si128((__m128i*)&blk[8 * i + 16 * 63], V63[i]);
	}
}
